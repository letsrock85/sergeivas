...
function handleObjectNode(node, scope) {
  ...
  for (const attr of node.attributes) {
    ...
    if (attr.type === "ObjectConditionalSplat") {
      ...
      if ($trace("object.conditional.splat.condition %O", condition), condition || condition === void 0) {
        const value = walk({ node: attr.value, scope });
        mapObjectSplat(value, scope, (node2) => {
          for (const name in node2.attributes) {
            if (!Object.hasOwn(node2.attributes, name))
              continue;
            const attribute = node2.attributes[name];
            if (condition)
