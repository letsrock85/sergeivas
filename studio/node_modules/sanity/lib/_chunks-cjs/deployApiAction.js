...
`), this.problemGroups.forEach((group) => {
      group.problems.forEach((problem) => {
        ...
        output.error(`  ${icon} ${upperFirst__default.default(problem.severity)}: ${getPath(group.path)}`), output.error(`  ${problem.message}`), problem.helpId && output.error(`  See ${generateHelpUrl.generateHelpUrl(problem.helpId)}`), output.error("");
      });
    });
  }
}
...
function createLiftTypeArrayError(index, parent, inlineType = "object", grandParent = "") {
  const helpUrl = generateHelpUrl.generateHelpUrl(SCHEMA_LIFT_ANONYMOUS_OBJECT_TYPE), context = [grandParent, parent].filter(Boolean).join("/");
