...
const CONNECTION_TIMEOUT$1 = 15 * 1e3, READ_TIMEOUT$1 = 3 * 60 * 1e3, request$1 = getIt([keepAlive(), promise()]);
async function downloadAsset(url2, fileName, fileType, outDir) {
  ...
  await withRetry(async () => {
    ...
    debug$1("Received asset %s with status code %d", normalizedFileName, response == null ? void 0 : response.statusCode), await pipeline(response.body, createWriteStream(assetFilePath));
  });
}
...
const CONNECTION_TIMEOUT = 15 * 1e3, READ_TIMEOUT = 3 * 60 * 1e3, request = getIt([keepAlive(), promise()]);
...
class PaginatedGetBackupStream extends Readable {
  constructor(client, projectId, datasetName, backupId, token) {
    super({ objectMode: !0 }), this.cursor = "", this.totalFiles = 0, this.client = client, this.projectId = projectId, this.datasetName = datasetName, this.backupId = backupId, this.token = token;
  }
  async _read() {
    try {
...
const debug = debug$3("sanity:backup"), DEFAULT_DOWNLOAD_CONCURRENCY = 10, MAX_DOWNLOAD_CONCURRENCY = 24, helpText$x = `
Options
...
function parseCliFlags$7(args) {
  return yargs(hideBin(args.argv || process.argv).slice(2)).options("backup-id", { type: "string" }).options("out", { type: "string" }).options("concurrency", { type: "number", default: DEFAULT_DOWNLOAD_CONCURRENCY }).options("overwrite", { type: "boolean", default: !1 }).argv;
}
...
`);
    ...
    docOutStream.end(), await finished(docOutStream), progressSpinner.set({ step: "Archiving files into a tarball...", update: !0 });
    try {
...
function parseCliFlags$6(args) {
  return yargs(hideBin(args.argv || process.argv).slice(2)).options("after", { type: "string" }).options("before", { type: "string" }).options("limit", { type: "number", default: DEFAULT_LIST_BACKUP_LIMIT, alias: "l" }).argv;
}
...
async function getBuildAction() {
  return (await import("./buildAction.js")).default;
}
const checkCommand = {
  name: "check",
  signature: "",
...
`, addCorsOriginCommand = {
  ...
  action: async (args, context) => {
    ...
    fs.existsSync(path$1.join(process.cwd(), origin)) && output.warn(`Origin "${origin}?" Remember to quote values (sanity cors add "*")`), await addCorsOrigin(origin, flags, context) && output.print("CORS origin added successfully");
  }
}, corsGroup = {
  name: "cors",
  signature: "[COMMAND]",
...
function parseCliFlags$3(args) {
  return yargs(hideBin(args.argv || process.argv).slice(2)).option("attach", { type: "string" }).option("list", { type: "boolean" }).option("limit", { type: "number" }).option("offset", { type: "number" }).option("skip-history", { type: "boolean" }).option("detach", { type: "boolean" }).argv;
}
const progress = (url2) => new Observable((observer) => {
  let progressSource = new EventSource(url2), stopped = !1;
  function onError(error) {
    progressSource && progressSource.close(), debug$2(`Error received: ${error}`), !stopped && (observer.next({ type: "reconnect" }), progressSource = new EventSource(url2));
  }
...
const exportDatasetCommand = {
  ...
  action: async (args, context) => {
    ...
    destinationPath || (destinationPath = await prompt.single({
      type: "input",
      message: "Output path:",
      default: path$1.join(workDir, `${dataset}.tar.gz`),
      filter: absolutify
    }));
...
async function getOutputPath(destination, dataset, prompt, flags) {
  ...
  if (!dstStats) {
    ...
    await fs$1.mkdir(createPath, { recursive: !0 });
  }
  const finalPath = looksLikeFile ? dstPath : path$1.join(dstPath, `${dataset}.tar.gz`);
...
const importDatasetCommand = {
  ...
  action: async (args, context) => {
    ...
    if (isUrl)
      ...
      const sourceFile = path$1.resolve(process.cwd(), file), fileStats = await fs$1.stat(sourceFile).catch(() => null);
      if (!fileStats)
        throw new Error(`${sourceFile} does not exist or is not readable`);
      sourceIsFolder = fileStats.isDirectory(), sourceIsFolder ? inputStream = sourceFile : (assetsBase = path$1.dirname(sourceFile), inputStream = await createReadStream(sourceFile));
    }
...
function getUrlStream(url2) {
  return getIt([promise({ onlyBody: !0 })])({ url: url2, stream: !0 });
}
...
`, deployCommand = {
  ...
  action: async (args, context) => (await import("./deployAction.js")).default(args, context),
  helpText: helpText$k
}, helpText$j = `
Examples
  sanity undeploy
`, undeployCommand = {
  ...
  action: async (args, context) => (await import("./undeployAction.js")).default(args, context),
  helpText: helpText$j
}, helpText$i = `
Notes
...
async function getDevAction() {
  return (await import("./devAction.js")).default;
}
const helpText$h = `
Options
...
`, createDocumentsCommand = {
  ...
  action: async (args, context) => {
    ...
    if ((replace || missing) && (operation = replace ? "createOrReplace" : "createIfNotExists"), file) {
      const contentPath = path$1.resolve(process.cwd(), file), content = json5.parse(await fs$1.readFile(contentPath, "utf8")), result = await writeDocuments(content, operation, client);
      output.print(getResultMessage(result, operation));
      return;
    }
    const docId = id || uuid(), ext = useJson5 ? "json5" : "json", tmpFile = path$1.join(os.tmpdir(), "sanity-cli", `${docId}.${ext}`), stringify = useJson5 ? json5.stringify : JSON.stringify, defaultValue = id && await client.getDocument(id) || { _id: docId, _type: "specify-me" };
    await fs$1.mkdir(path$1.join(os.tmpdir(), "sanity-cli"), { recursive: !0 }), await fs$1.writeFile(tmpFile, stringify(defaultValue, null, 2), "utf8");
    const editor = getEditor();
    watch ? (registerUnlinkOnSigInt(tmpFile), output.print(`Watch mode: ${tmpFile}`), output.print("Watch mode: Will write documents on each save."), output.print("Watch mode: Press Ctrl + C to cancel watch mode."), chokidar.watch(tmpFile).on("change", () => (output.print(""), readAndPerformCreatesFromFile(tmpFile))), execa(editor.bin, editor.args.concat(tmpFile), { stdio: "inherit" })) : (execa.sync(editor.bin, editor.args.concat(tmpFile), { stdio: "inherit" }), await readAndPerformCreatesFromFile(tmpFile), await fs$1.unlink(tmpFile).catch(noop$1));
...
`, deleteDocumentsCommand = {
  ...
  action: async (args, context) => {
    ...
    try {
      ...
      deleted.length > 0 && output.print(`Deleted ${deleted.length} ${pluralize("document", deleted.length)}`), notFound.length > 0 && output.error(
...
function parseCliFlags$1(args) {
  ...
  return yargs(hideBin(args.argv || process.argv).slice(2)).option("pretty", { type: "boolean", default: !1 }).option("dataset", { type: "string" }).option("project", { type: "string" }).option("anonymous", { type: "boolean", default: !1 }).option("api-version", { type: "string", default: fallbackApiVersion }).argv;
}
...
`, listGraphQLAPIsCommand = {
  ...
  action: async (args, context) => {
    ...
    output.print(`Opening ${manageUrl}`), open(manageUrl);
  }
}, deleteHookCommand = {
  name: "delete",
  group: "hook",
...
function printMessage(message, context, options) {
  ...
  output.print(`Date: ${message.createdAt}`), output.print(`Status: ${message.status}`), output.print(`Result code: ${message.resultCode}`), message.failureCount > 0 && output.print(`Failures: ${message.failureCount}`), detailed && (output.print("Payload:"), output.print(inspect(JSON.parse(message.payload), { colors: !0 }))), detailed && message.attempts && (output.print("Attempts:"), message.attempts.forEach((attempt) => {
...
`, TEMPLATES = [
  ...
  action: async (args, context) => {
    ...
    await writeFile(definitionFile, renderedTemplate), output.print(), output.print(`${chalk.green("\u2713")} Migration created!`), output.print(), output.print("Next steps:"), output.print(
      `Open ${chalk.bold(
        definitionFile
...
function resolveMigrationScript(workDir, migrationName) {
  return [migrationName, path.join(migrationName, "index")].flatMap(
...
async function resolveMigrations(workDir) {
  ...
  const migrationsDir = path.join(workDir, MIGRATIONS_DIRECTORY), migrationEntries = await readdir(migrationsDir, { withFileTypes: !0 }), migrations = [];
...
function convertToTree(nodes) {
  ...
  function addNode(node, tree = root) {
    ...
    tree.children || (tree.children = {}), key in tree.children || (tree.children[key] = {}), addNode({ ...node, path: rest }, tree.children[key]);
  }
...
function parseCliFlags(args) {
  return yargs(hideBin(args.argv || process.argv).slice(2)).options("dry-run", { type: "boolean", default: !0 }).options("concurrency", { type: "number", default: DEFAULT_MUTATION_CONCURRENCY }).options("progress", { type: "boolean", default: !0 }).options("dataset", { type: "string" }).options("from-export", { type: "string" }).options("project", { type: "string" }).options("confirm", { type: "boolean", default: !0 }).argv;
}
const runMigrationCommand = {
  ...
  // eslint-disable-next-line max-statements
  action: async (args, context) => {
    const { apiClient, output, prompt, chalk, workDir } = context, [id] = args.argsWithoutOptions, migrationsDirectoryPath = path.join(workDir, MIGRATIONS_DIRECTORY), flags = await parseCliFlags(args), fromExport = flags.fromExport, dry = flags.dryRun, dataset = flags.dataset, project = flags.project;
 ...
 - `)}`
    ...
    await run({ api: apiConfig, concurrency, onProgress: createProgress(spinner) }, migration), spinner.stop();
...
`, listUsersCommand = {
  ...
  action: async (args, context) => {
    ...
    })).filter((member) => !member.isRobot || robots), ...pendingInvitations], ordered = sortBy(
