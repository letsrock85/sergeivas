...
import { g as getClientUrl, d as debug } from "./_internal.js";
...
import { g as getGraphQLAPIs } from "./getGraphQLAPIs.js";
...
class SchemaError extends Error {
  constructor(problemGroups) {
    super("Schema errors encountered"), this.problemGroups = problemGroups;
  }
  print(output) {
...
function extractFromSanitySchema(sanitySchema, extractOptions = {}) {
  ...
  const sanityTypes = ((_a = sanitySchema._original) == null ? void 0 : _a.types) || [], typeNames = sanitySchema.getTypeNames(), unionTypes = [], types = [];
  ...
  return { types: [...types, ...unionTypes], interfaces: [getDocumentInterfaceDefinition()] };
...
class HelpfulError extends Error {
  constructor(message, helpUrl) {
    super(message), this.helpUrl = helpUrl;
  }
}
...
function generateTypeFilters$2(types) {
  return types.filter(isNonUnion).filter(
...
function generateTypeQueries$2(types, filters) {
  const queries = [], queryable = types.filter(isNonUnion).filter(
    (type) => type.type === "Object" && type.interfaces && type.interfaces.includes("Document")
  );
...
function generateTypeFilters$1(types) {
  const builtInTypeKeys = Object.keys(filterCreators$1), builtinTypeValues = Object.values(filterCreators$1), objectTypes = types.filter(isNonUnion).filter(
    (type) => type.type === "Object" && !["Block", "Span"].includes(type.name) && !type.interfaces && !builtInTypeKeys.includes(type.type)
  ), unionTypes = types.filter(isUnion).map((type) => type.name), documentTypes = types.filter(isNonUnion).filter(
    (type) => type.type === "Object" && type.interfaces && type.interfaces.includes("Document")
...
function generateTypeSortings$1(types) {
  ...
  ), documentTypes = types.filter(isDocumentType), hasFields = (type) => type.fields.length > 0, objectTypeSortings = createObjectTypeSortings$1(objectTypes), documentTypeSortings = createDocumentTypeSortings$1(documentTypes);
...
function generateTypeFilters(types, options) {
  const { filterSuffix } = options || {}, builtInTypeKeys = Object.keys(filterCreators), builtinTypeValues = Object.values(filterCreators), objectTypes = types.filter(isNonUnion).filter(
    (type) => type.type === "Object" && !["Block", "Span"].includes(type.name) && // TODO: What do we do with blocks?
    !type.interfaces && !builtInTypeKeys.includes(type.type)
  ), unionTypes = types.filter(isUnion).map((type) => type.name), documentTypes = types.filter(
...
function generateTypeQueries(types, sortings, options) {
  const { filterSuffix } = options || {}, queries = [], documentTypes = types.filter(isDocumentType), documentsFilter = `_type in [${documentTypes.map(
    (docType) => JSON.stringify(docType.originalName || docType.name)
  ).join(", ")}]`, documentInterface = types.find((type) => type.name === "Document");
...
var gen3 = (extracted, options) => {
  ...
  if (!documentInterface || isUnion(documentInterface))
    throw new Error("Failed to find document interface");
  const types = [...extracted.types, documentInterface], filters = generateTypeFilters(types, options), sortings = generateTypeSortings(types), queries = generateTypeQueries(
    types,
    sortings.filter((node) => node.kind === "InputObject"),
    options
  );
...
async function deployGraphQLApiAction(args, context) {
  ...
  const client = apiClient({
    ...
    requireProject: !1
  }).config({ apiVersion: "2023-08-01" }), apiDefs = await getGraphQLAPIs(context), hasMultipleApis = apiDefs.length > 1 || flags.api && flags.api.length > 1, usedFlags = [
    datasetFlag && "--dataset",
    tagFlag && "--tag",
  ...
  for (const apiDef of apiDefs) {
    if (onlyApis && (!apiDef.id || !onlyApis.includes(apiDef.id)))
      continue;
    index++;
    const dataset = datasetFlag || apiDef.dataset, tag = tagFlag || apiDef.tag || "default", { projectId, playground, nonNullDocumentFields, schema } = apiDef, apiName = [dataset, tag].join("/");
...
function parseCliFlags(args) {
  return yargs(hideBin(args.argv || process.argv).slice(2)).option("tag", { type: "string" }).option("dataset", { type: "string" }).option("api", { type: "string", array: !0 }).option("dry-run", { type: "boolean", default: !1 }).option("generation", { type: "string" }).option("non-null-document-fields", { type: "boolean" }).option("playground", { type: "boolean" }).option("with-union-cache", { type: "boolean" }).option("force", { type: "boolean" }).argv;
}
