...
import { c as convertToTree, m as maxKeyLength, f as formatTree } from "./_internal.js";
...
const json = async ({ output, worker }) => {
...
}, isTty = isatty(1), levelValues = { error: 0, warning: 1, info: 2 }, count = (amount, subject) => `${amount.toLocaleString("en-US")} ${amount === 1 ? subject.substring(0, subject.length - 1) : subject}`, percentageFormatter = new Intl.NumberFormat("en-US", {
  style: "percent",
  ...
  return [
    ...
    levelValue >= levelValues.warning && `${logSymbols.warning} Warnings: ${count(warnings.documents, "documents")}, ${count(
      warnings.markers,
      "warnings"
    )}`,
    levelValue >= levelValues.info && `${logSymbols.info} Info:     ${count(infos.documents, "documents")}, ${count(
      infos.documents,
      "markers"
    )}`
  ].filter(Boolean).join(`
...
}, link = (text, url) => isTty ? `\x1B]8;;${url}\x07${text}\x1B]8;;\x07` : chalk.underline(text), compareLevels = (a, b) => levelValues[a.level] - levelValues[b.level], formatRootErrors = (root, hasChildren, paddingLength) => {
  if (!root.nodes)
    return "";
  const [first, ...rest] = root.nodes.slice().sort(compareLevels);
  if (!first)
    return "";
  const firstElbow = hasChildren ? "\u2502 " : "\u2514\u2500", firstPadding = ".".repeat(paddingLength - 6), firstLine = `${firstElbow} (root) ${firstPadding} ${logSymbols[first.level]} ${first.message}`, subsequentPadding = " ".repeat(paddingLength + 2), subsequentElbow = hasChildren ? "\u2502 " : "  ", restOfLines = rest.map(
    (marker) => `${subsequentElbow}${subsequentPadding} ${logSymbols[marker.level]} ${marker.message}`
  ).join(`
`);
...
function formatDocumentValidation({
  ...
  const tree = convertToTree(markers), documentTypeHeader = isTty ? chalk.bgWhite(chalk.black(` ${documentType} `)) : `[${documentType}]`, header = `${levelHeaders[level]} ${documentTypeHeader} ${intentUrl ? link(documentId, intentUrl) : chalk.underline(documentId)}`, paddingLength = Math.max(maxKeyLength(tree.children) + 2, 30), childErrors = formatTree({
    node: tree.children,
    paddingLength,
...
function validateDocuments(options) {
  ...
  const {
    ...
    ndjsonFilePath
  } = options, rootPkgPath = (_a = readPkgUp.sync({ cwd: __dirname })) == null ? void 0 : _a.path;
  if (!rootPkgPath)
...
async function validateAction(args, { apiClient, workDir, output, prompt }) {
  ...
  if (!(flags.yes || flags.y) && (output.print(
    `${chalk.yellow(`${logSymbols.warning} Warning:`)} This command ${flags.file ? "reads all documents from your input file" : "downloads all documents from your dataset"} and processes them through your local schema within a simulated browser environment.
`
